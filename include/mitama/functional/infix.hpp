...
public:
  ...
  constexpr explicit infixed(tag, Fn&& fn, Args&&... args)
      : fn(std::forward<Fn>(fn)), partial(std::forward<Args>(args)...)
  {
  }
  ...
  constexpr auto operator()(Remaining&&... remaining) const
  {
    return std::apply(
        ...
        {
          return std::invoke(
              fn, std::forward<Partial>(partial)...,
  ...
  constexpr auto operator()(Remaining&&... remaining) const
  {
    return std::apply(
        [&](auto&&... partial)
        {
          return infixed{ std::forward<F>(fn),
...
infixed(F&&, Partial&&...) -> infixed<0, F, Partial...>;
...
infix(auto&& fn, auto&&... partial)
{
  return infixed{ std::integral_constant<std::size_t, Precedence>{},
